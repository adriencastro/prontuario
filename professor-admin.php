...
try {
    ...
    $sql = "SELECT id, nome, orientacao, sessao FROM orientacao ORDER BY id DESC";
    $stmt = $pdo->query($sql);
...
<body>
    <header>
        <nav id="navbar">
            <i class="fa-solid fa-user" id="nav_logo"> Área do Professor</i>
            <ul id="nav_list">
               <!-- <li class="nav-item">
                ...
                <li class="nav-item active">
                    <a href="professor-usr.php">Professor</a>
                </li>
            </ul>

            <?php if ($estaLogado): ?>
                <p>Logado como: <strong><?= htmlspecialchars($_SESSION['usuario']); ?></strong></p>
                <a href="logout-professor.php"><button id="logoutButton" class="btn-default">Logout</button> </a>
            <?php else: ?>
                <a href="login-professor.php"><button id="logoutButton" class="btn-default">Login</button></a>
            <?php endif; ?>
    ...
    <script>
        async function enviarFormulario(event) {
            ...
            try {
                const resposta = await fetch('inserir_orientacao.php', {
                    method: 'POST',
                    body: formData
                });
    ...
    <div class="container">
        ...
        <div class="sidebar">
            ...
            <div class="card novo-paciente" onclick="mostrarFormulario()">
                <h3>Nova Orientação</h3>
                <p>Clique para adicionar uma nova orientação</p>
        ...
        <div class="main-content">
            ...
            <div class="form-adicionar-paciente" id="formAdicionarPaciente">
                <h3>Adicionar Nova Orientação</h3>
                    ...
                    <textarea id="orientacao" name="orientacao" rows="4"></textarea>
...
<?php foreach ($registros as $registro): ?>
           ...
           <!-- Cards de Sessões -->
            <div class="card">
                <h3>Sessão:<?= htmlspecialchars($registro['sessao']); ?> <br>Nome: <?= htmlspecialchars($registro['nome']); ?></h3>
                <p><strong>Orientação:</strong><?= htmlspecialchars($registro['orientacao']); ?></p>
            </div>
